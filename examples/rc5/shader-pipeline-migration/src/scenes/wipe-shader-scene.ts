...
import { WipeShader, WipeShaderController } from '../shaders/wipe-shader';

const IMAGE_ASSET_KEY = 'BG';

export class WipeShaderScene extends Phaser.Scene {
  constructor() {
    super({ key: 'WipeShaderScene' });
  }

  preload(): void {
    // load in data
    this.load.image(IMAGE_ASSET_KEY, 'assets/images/bg.png');
    // load our custom shader program
    // NOTE: the key here is the key we provide in our BaseFilterShader class
    this.load.glsl('wipe', 'assets/shader/wipe.frag');
  }
  ...
  create(): void {
    ...
    if (!renderer.renderNodes.hasNode('Wipe')) {
      ...
      renderer.renderNodes.addNodeConstructor('Wipe', WipeShader);
    }
    ...
    const wipeController = new WipeShaderController(this.cameras.main);

    // add filter controller to the main camera
    this.cameras.main.filters.external.add(wipeController);

    // finally, here is an example of how we can pass dynamic values to our shader as uniform values.
    // this is done by setting the values on the controller associated with the filter we set on our game object/camera.

    // here, we use a tween to update our progress property on our wipe controller
    this.tweens.add({
      targets: wipeController,
      progress: 1,
      duration: 2000,
      delay: 1000,
    });
    ...
    this.input.keyboard?.once('keydown-SPACE', () => {
      this.scene.start('NoiseShaderScene');
    });
  }
}
